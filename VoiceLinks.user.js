...
// @updateURL   https://github.com/aaaaltaaaa/VoiceLinks/raw/master/VoiceLinks.user.js
// @downloadURL https://github.com/aaaaltaaaa/VoiceLinks/raw/master/VoiceLinks.user.js
...
.voicelink : hover {
  ...
  const Parser = {
      ...
      wrapRJCode: function (rjCode) {
          ...
          e.href = `https://www.dlsite.com/maniax/work/=/product_id/${rjCode}.html/?locale=zh_CN`
          e.innerHTML = rjCode;
          e.target = "_blank";
          e.rel = "noreferrer";
      ...
      linkify: function (textNode) {
          ...
          let match;
          if (match = RJ_REGEX.exec(nodeOriginalText)) {
              var len =8
              if (parseInt(nodeOriginalText.substring(match.index+8,match.index+10)))
              {
                 match[0]=nodeOriginalText.substring(match.index,match.index+10)
                 len=10
              }
              matches.push({
                  index: match.index,
          ...
          for (let i = 0; i < matches.length; ++i) {
              ...
              let substring;
              if (substring = nodeOriginalText.substring(matches[i].index + matches[i].len, upper)) {
      ...
      makePopup: function (e, rjCode) {
          ...
          DLsite.request(rjCode, function (workInfo) {
              if (workInfo === null){
                  ...
                  let html = `
                      <div>
                          <div class='标题'>${workInfo.title}</div>
                          <br />
                          社团: <a>${workInfo.circle}</a>
                          <br />
                  `;
                  if (workInfo.date)
                      html += `发售日期: <a>${workInfo.date}</a> <br />`;
                  else if (workInfo.dateAnnounce)
                      html += `Scheduled Release: <a>${workInfo.dateAnnounce}</a> <br />`;

                  html += `年龄限制: <a>${workInfo.rating}</a><br />`

                  if (workInfo.cv)
                      html += `声优: <a>${workInfo.cv}</a> <br />`;
                  ...
                  if (workInfo.filesize)
                      html += `文件大小: ${workInfo.filesize}<br />`;
  ...
  const DLsite = {
      parseWorkDOM: function (dom, rj) {
          ...
          workInfo.img =dom.querySelector(".active > picture > img")['srcset'];
          console.log(workInfo.img)
          ...
          for (var i = 0, ii = table_outline.rows.length; i < ii; i++) {
              ...
              switch (true) {
                  ...
                  case (row_header.includes("分类")):
                      workInfo.tags = [...row_data.querySelectorAll("a")].map(a => { return a.innerText });
                      break;
                  case (row_header.includes("声优")):
                      workInfo.cv = row_data.innerText;
                      break;
                  case (row_header.includes("文件容量")):
                      workInfo.filesize = row_data.innerText.replace("总计", "").trim();
                      break;
                  default:
                      break;
              }
